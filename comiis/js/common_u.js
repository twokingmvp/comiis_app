/* แก้ปัญหา Unable to preventDefault (Passive Event) สำหรับเทมเพลตเก่า */
(function() {
	...
	converthtml : function() {
		...
		if(lastpage) {
			...
			for(var i=1; i<=lastpage; i++) {
				selector += '<option value="'+i+'" '+ (i == curpage ? 'selected' : '') +'>หน้าที่'+i+'</option>';
			}
			selector += '</select>';
			selector += 'หน้าที่ '+curpage+'';
		}
		$('div.pg').removeClass('pg').addClass('comiis_page bg_f').html('<a href="'+ prevpagehref +'" class="bg_f b_ok">ย้อนกลับ</a>'+ selector +'<a href="'+ nextpagehref +'" class="bg_f b_ok">ถัดไป</a>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof comiis_share_box_close === 'function' && !$('.comiis_share_box').is(':hidden')){
		...
		if(typeof pop == 'string' && pop.indexOf("<a ") >= 0 && pop.indexOf("<form ") < 0 && pop.indexOf("<select ") < 0 && pop.indexOf('<button') < 0) {
			type = 'alerts';
		}
		if(type == 'alert') {
		...
		if(typeof pop == 'string') {
			...
			if(type == 'alerts') {
				pop = '<div class="comiis_tip bg_f cl"><dt class="f_b"><p>'+ pop +'</p></dt><dd class="b_t cl"><a href="javascript:;" onclick="popup.close();" class="tip_btn tip_all bg_f f_b">ตกลง</a></dd></div>'
			} else if(type == 'confirm') {			
			if(comiis_post_btnwz == 1){
				pop = '<div class="comiis_tip bg_f cl"><dt class="f_b"><p>'+ pop +'</p></dt><dd class="b_t cl"><a href="javascript:;" onclick="popup.close();" class="tip_btn bg_f f_b">ยกเลิก</a><a href="'+ url +'" class="tip_btn bg_f f_0"><span class="tip_lx">ตกลง</span></a></dd></div>'
			}else{
				pop = '<div class="comiis_tip bg_f cl"><dt class="f_b"><p>'+ pop +'</p></dt><dd class="b_t cl"><a href="'+ url +'" class="tip_btn bg_f f_0">ตกลง</a><a href="javascript:;" onclick="popup.close();" class="tip_btn bg_f f_b"><span class="tip_lx">ยกเลิก</span></a></dd></div>'
			}			
			}
		...
		if(comiis_textarea && $('#' + pop.attr('id') + ' textarea').attr('name') == 'givemessage'){
			comiis_textarea = 0;
		}
		...
		if(POPMENU[pop.attr('id')]) {
			...
			$('body').append('<div class="dialogbox" id="'+ pop.attr('id') +'_popmenu" style="height:'+ pop.height() +'px;width:'+ (comiis_textarea ? '90%' : pop.width()) +'px;">'+ html_box +'</div>');
		}
...
var formdialog = {
	init : function() {
		$(document).on('click', '.formdialog', function() {
			...
			if(formobj.attr('onsubmits')){
				eval('var $return_data=' + formobj.attr('onsubmits') + ';');
...
var Comiis_Touch = {
	...
	onTouchMove : function(e){
		...
		if (Math.abs(Y) > Math.abs(X) && Y > 0 && $(window).scrollTop() <= 0 && (Comiis_Touch.Touchon ==0 || Comiis_Touch.Touchon == 1) && !$('.comiis_body').hasClass('comiis_showleftnv') && Comiis_Touch_on != 2) {
			...
			Comiis_Touch.TouchMoves = -Y;
		}else if (Math.abs(X) > Math.abs(Y) && X > 0 && $(document).scrollLeft() <= 0 && Comiis_Touch.TouchscrollTop == $(window).scrollTop() && (Comiis_Touch.Touchon ==0 || Comiis_Touch.Touchon == 3) && Comiis_Touch_on != 3) {
			e.preventDefault();
	...
	init : function() {
		$('img').on('error', function() {
			if ($(this).attr('src').indexOf('.webp') !== -1) {
				...
				$(this).off('error').attr('src', $(this).attr('src').replace('.jpg', '.webp'));
			}
		});
	}
};
var comiis_position_state = 0;
var coiis_a_ajax_data = [];
$(document).ready(function() {
	$(document).on('click', 'a', function(e) {
		...
		if(typeof(href) != "undefined" && href.indexOf('javascript:') === -1 && href.indexOf('#') === -1 && !obj.hasClass('dialog')){
			e.preventDefault();
			var comiis_ajax = obj.attr("comiis_ajax");
			if(comiis_ajax){
				var comiis_t_ajax = obj.attr("comiis_t_ajax");
				if(comiis_t_ajax && $(comiis_t_ajax).find(".swiper-wrapper").length){
					comiis_t_ajax += ' .swiper-wrapper';
				}
				var coiis_a_ajax_md5 = hash(href.replace(/[\?|&]mobile=2/g, ''), 16);
				$('.comiis_loadings').show();
				if(coiis_a_ajax_data[coiis_a_ajax_md5]){
					$(comiis_ajax).html(coiis_a_ajax_data[coiis_a_ajax_md5]['comiis_ajax']);
					$(comiis_t_ajax).html(coiis_a_ajax_data[coiis_a_ajax_md5]['comiis_t_ajax']);
					$('.comiis_loadings').hide();
					if($('div.pg').length > 0) {
						page.converthtml();
					}
					if(coiis_a_ajax_data[coiis_a_ajax_md5]['comiis_js_ajax']){
						$('body').append(coiis_a_ajax_data[coiis_a_ajax_md5]['comiis_js_ajax']);
					}
				}else{
					$.ajax({
						type:'GET',
						...
						var html = $("<div></div>"), js= '';
						var data = s.lastChild.firstChild.nodeValue;
						if(data.indexOf("</script>") != -1){
							js = data.match(/\<script(.*?)\>((.|\n|\r)*?)\<\/script\>/gi).join("\n");
							data = data.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/ig, '');
						}
						html.append(data);
						var htm = html.find(comiis_ajax).html();
						$(comiis_ajax).html(htm);
						if(comiis_t_ajax){
							var t_htm = html.find(comiis_t_ajax).html();
							$(comiis_t_ajax).html(t_htm);
						}
						$('.comiis_loadings').hide();
						if($('div.pg').length > 0) {
							page.converthtml();
						...
						coiis_a_ajax_data[coiis_a_ajax_md5] = {
							"comiis_ajax" : htm,
							"comiis_t_ajax" : t_htm,
							"comiis_js_ajax" : js
						}
	...
	if($('#comiis_allreplies').length > 0){
		$(document).on('click', '.comiis_position_key', function() {
			if(comiis_position_state == 0){
				if($('.comiis_head_hidden').length > 0){
					var comiis_headheight = 0;
				}else{
					var comiis_headheight = 59;
				}
				$('html,body').animate({scrollTop: $('#comiis_allreplies').offset().top - comiis_headheight}, 800)
			}else{
	...
	$(window).scroll(function() {
		...
		if($('.comiis_view_topuser').length > 0 && $('.comiis_postli_top').length > 0){
			if(comiis_scrollTop_num >= $('.comiis_postli_top').offset().top){
				$('#comiis_head').addClass('comiis_head_show_avatar');
			}else{
				$('#comiis_head').removeClass('comiis_head_show_avatar');
			}
		}
		if($('#comiis_allreplies').length > 0){
			if(comiis_scrollTop_num + $(window).height()  >= $('#comiis_allreplies').offset().top + 80){
				if(comiis_position_state == 0){
					comiis_position_state = 1;
					$('.comiis_position_key i').html('&#xe633;');
				}
			}else{
				if(comiis_position_state == 1){
					comiis_position_state = 0;
					$('.comiis_position_key i').html('&#xe680;');
				}
			}
		}
...
var cat1 = Comiis_Touch.init('html', {
	...
	if(comiis_leftnv_id == 0){
	...
	}else if(comiis_leftnv_id == 3){
		if(!$('.comiis_body').hasClass('comiis_showleftnv')){
			if(Comiis_Touch.TouchDirection == 'right'){
				Comiis_Touch_endtime = new Date().getTime();
				if(Comiis_Touch.TouchMoves > 50){
					Comiis_Touch.Touchon = 3;
					Comiis_Touch_openleftnav = 1;
		...
		}else if(Comiis_Touch.TouchDirection == 'left' && $('.comiis_body').hasClass('comiis_showleftnv')){
			Comiis_Touch_endtime = new Date().getTime();
			if(Comiis_Touch.TouchMoves > 50){
				Comiis_Touch.Touchon = 4;
				Comiis_Touch_openleftnav = 1;
...
$(document).on('click', '.comiis_dateshow:text,.comiis_dateshow_nt:text', function() {
	...
	var comiis_date_start = comiis_y + 5;
	for(var k = comiis_date_start; k >= 1949; k--) {
    ...
    if(comiis_post_btnwz == 1){
        datehtml += '</div>' + 
        '<dd class="b_t cl">' + 
        '<a href="javascript:;" onclick="comiis_closedate()" class="tip_btn bg_f f_b">ยกเลิก</a>' + 
        '<a href="javascript:;" onclick="comiis_setdate(\'' + id + '\', \'' + istime + '\');" class="tip_btn bg_f f_0"><span class="tip_lx">ตกลง</span></a>' + 
        '</dd>' + 
        '</div>';
    }else{
        datehtml += '</div>' + 
        '<dd class="b_t cl">' + 
        '<a href="javascript:;" onclick="comiis_setdate(\'' + id + '\', \'' + istime + '\');" class="tip_btn bg_f f_0">ตกลง</a>' + 
        '<a href="javascript:;" onclick="comiis_closedate()" class="tip_btn bg_f f_b"><span class="tip_lx">ยกเลิก</span></a>' + 
        '</dd>' + 
        '</div>';
    }
	popup.open(datehtml);
...
function comiis_miniprogram(a, b){
	if(a.indexOf("appid=") != -1){
		if(window.__wxjs_environment === 'miniprogram'){
			wx.miniProgram.navigateTo({url: '/pages/comiis/open?'+ a});
		}else{
			if(b){
				window.location.href = b;
			}else{
				popup.open('小程序专用连接, 请在小程序中打开. ', 'alerts');
			}
		}
	}
}
