...
function tpl_quote() {
<!--{/eval}-->
<!--{block return}--><div class="comiis_quote bg_h b_dashed f_c"><blockquote><font size="2">มีคำกล่าวว่า</font> \\1</blockquote></div><!--{/block}-->
<!--{eval}-->
	return $return;
}
function tpl_free() {
...
<!--{block return}-->
	<div class="comiis_attach bg_e b_ok cl">
		<a {if !$attach['price'] || $attach['payed']}href="forum.php?mod=attachment{$is_archive}&aid=$aidencode"{else}href="forum.php?mod=misc&action=attachpay&aid=$attach[aid]&tid=$attach['tid']" class="dialog"{/if}>
		...
		<div class="attach_txt bg_f b_ok">
			<!--{if $attach['price']}-->
				<h2 class="{if $attach['description']}b_b {/if}f_a">
					<!--{if !$attach['payed']}--><a href="forum.php?mod=misc&action=attachpay&aid=$attach[aid]&tid=$attach['tid']" class="dialog">[{lang attachment_buy}]</a><!--{/if}-->
					<a href="forum.php?mod=misc&action=viewattachpayments&aid=$attach[aid]">[{lang pay_view}]</a>
...
function imagelist($attach, $firstpost = 0) {
	...
	if($guestviewthumb){
		if ($post['first'] == 0 && $comiis_app_switch['comiis_aimg_show'] == 1 || $comiis_app_switch['comiis_aimg_oneshow'] == 1 && $post['first'] == 1) {
			if (count($post['imagelist']) == 1){
				$mobilethumburl = comiis_getforumimg($attach['aid'], 0, 300, 300, 'fixnone');		
			}else{
				$mobilethumburl = comiis_getforumimg($attach['aid'], 0, 350, 320, 2);		
			}
		}else{
		$mobilethumburl = comiis_getforumimg($attach['aid'], 0, 300, 300, 'fixnone');
		}		
	}else{
		$mobilethumburl = $attach['attachimg'] && $_G['setting']['showimages'] && (!$attach['price'] || $attach['payed']) && ($_G['group']['allowgetimage'] || $_G['uid'] == $attach['uid']) ? (((($comiis_app_switch['comiis_aimg_oneshow'] == 0 && $post['first'] == 1) || ($comiis_app_switch['comiis_aimg_show'] == 0  && $post['first'] == 0)) || count($aimgs[$post['pid']]) == 1) ? ($comiis_app_switch['comiis_view_imgdata'] == 1 ? $attach['url'].$attach[attachment] : comiis_getforumimg($attach['aid'], 0, 600, 1000, 'fixnone')) : comiis_getforumimg($attach['aid'], 0, 350, 320, 2)) : '' ;
	}
<!--{/eval}-->
<!--{block return}-->
...
function attachinpost($attach, $post) {
	...
	if($guestviewthumb){
		...
		$mobilethumburl = $attach['attachimg'] && $_G['setting']['showimages'] && (!$attach['price'] || $attach['payed']) && ($_G['group']['allowgetimage'] || $_G['uid'] == $attach['uid']) ? ($comiis_app_switch['comiis_view_imgdata'] == 1 ? $attach['url'].$attach[attachment] : comiis_getforumimg($attach['aid'], 0, 600, 1000, 'fixnone')) : comiis_getforumimg($attach['aid'], 0, 160, 300, 'fixnone') ;
	}	
	$is_archive = $_G['forum_thread']['is_archived'] ? '&fid='.$_G['fid'].'&archiveid='.$_G['forum_thread'][archiveid] : '';
	$attach['refcheck'] = (!$attach['remote'] && $_G['setting']['attachrefcheck']) || ($attach['remote'] && ($_G['setting']['ftp']['hideurl'] || ($attach['isimage'] && $_G['setting']['attachimgpost'] && strtolower(substr($_G['setting']['ftp']['attachurl'], 0, 3)) == 'ftp')));
	$aidencode = packaids($attach);
