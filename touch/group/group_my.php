...
<div class="comiis_topnv bg_f b_b">
	<ul class="comiis_flex">
		<li class="flex{if $actives[groupthread]} f_0{/if}">{if $actives[groupthread]}<em class="bg_0"></em>{/if}<a href="group.php?mod=my&view=groupthread"{if !$actives[groupthread]} class="f_c"{/if}>กระทู้ในกลุ่ม</a></li>
		<li class="flex{if $actives[mythread]} f_0{/if}">{if $actives[mythread]}<em class="bg_0"></em>{/if}<a href="group.php?mod=my&view=mythread"{if !$actives[mythread]} class="f_c"{/if}>{lang my_thread}</a></li>
		<li class="flex{if $actives[join]} f_0{/if}">{if $actives[join]}<em class="bg_0"></em>{/if}<a href="group.php?mod=my&view=join"{if !$actives[join]} class="f_c"{/if}>กลุ่มของฉัน</a></li>
		<li class="flex{if $actives[manager]} f_0{/if}">{if $actives[manager]}<em class="bg_0"></em>{/if}<a href="group.php?mod=my&view=manager"{if !$actives[manager]} class="f_c"{/if}>จัดการกลุ่ม</a></li>
	</ul>
</div>
