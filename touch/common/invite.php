...
var filterUser=[];
var filterUsers = [{$inviteduids}];
for(var id in filterUsers) {
	filterUser[filterUsers['id']] = filterUsers['id'];
}
function comiis_friend_loadpage(){
	...
	if(comiis_friend_page < 5  && comiis_exit == 0){
		$.ajax({
			type:'GET',
			url: 'home.php?mod=spacecp&ac=friend&op=getinviteuser&inajax=1&page=' + comiis_friend_page + '&gid=-1&at=&username=&{echo time();}',
			dataType:'xml',
		}).success(function(s) {
			if(s.lastChild.firstChild.nodeValue.length){
				...
				for(i in comiis_friend_data['userdata']) {
					if(typeof filterUser[i] != 'undefined') {
						continue;
					}
					maxfriendnums++;
				...
				$('#comiis_friendn').text(maxfriendnums);
