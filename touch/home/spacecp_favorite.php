...
<!--{if $_GET['op'] == 'delete'}-->
	<form id="favoriteform_{$favid}" name="favoriteform_{$favid}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=favorite&op=delete&favid=$favid&type=$_GET[type]&mobile=2">
		...
		<input type="hidden" name="handlekey" value="{if ($_GET['type'] == 'forum' || $_GET['type'] == 'group') && $_GET['formhash']}favorite_del{else}comiis{/if}" />
		<dt><p style="text-align:center;"><!--{if $_GET[type] == 'forum'}-->{$comiis_lang['all10']}?<!--{elseif $_GET[type] == 'thread'}-->{lang delete_favorite_message}<!--{else}-->{$comiis_lang['all13']}{$comiis_lang['all11']}<!--{/if}--></p></dt>
		<dd class="b_t cl">
