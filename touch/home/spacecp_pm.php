...
<!--{elseif $_GET['op'] == 'pm_report'}-->
		<div id="$pmid">
			<form id="pmreportform_{$pmid}" name="pmreportform_{$pmid}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=pm&op=pm_report&pmid=$pmid">
				...
				<input type="hidden" name="formhash" value="{FORMHASH}" />
					<div class="comiis_tip bg_f cl">
						<dt class="f_b"><p>{lang determine_report_pm}</p></dt>
						<dd class="b_t cl">
								<!--{if $comiis_app_switch['comiis_post_btnwz'] == 1}-->
										<a href="javascript:;" onclick="popup.close();" class="tip_btn bg_f f_b">{lang cancel}</a>
										<button type="submit" name="pmreportsubmit_btn" value="true" class="formdialog tip_btn bg_f f_0"><span class="tip_lx">{lang determine}</span></button>
								<!--{else}-->
										<input type="submit" name="pmreportsubmit_btn" value="true" class="formdialog tip_btn bg_f f_0">
										<a href="javascript:;" onclick="popup.close();" class="tip_btn bg_f f_b"><span class="tip_lx">{lang cancel}</span></a>	
								<!--{/if}-->
						</dd>
					</div>
...
<form id="pmform_{$pmid}" name="pmform_{$pmid}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=pm&op=send&touid=$touid&pmid=$pmid&mobile=2" >
	...
	<div class="comiis_btnbox cl">
		<button id="pmsubmit_btn" class="<!--{if !$_GET['inajax']}-->formdialog <!--{/if}-->comiis_btn bg_c f_f">{lang sendpm}</button>
	</div>
</form>
<!--{if $_GET['inajax']}-->
