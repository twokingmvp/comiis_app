...
<style>.comiis_footer_scroll {bottom:115px;}</style>
<!--{if $_GET['op']=='edit' || $_GET['op']=='editpic'}-->
	<div class="comiis_topnv bg_f b_b">
		<ul class="comiis_flex">
			...
			<li class="flex{if $_GET['op']=='edit'} f_0{/if}">{if $_GET['op']=='edit'}<em class="bg_0"></em>{/if}<a href="home.php?mod=spacecp&ac=album&op=edit&albumid=$albumid"{if $_GET['op']!='edit'} class="f_c"{/if}>{lang edit_album_information}</a></li>
			<li class="flex{if $_GET['op']=='editpic'} f_0{/if}">{if $_GET['op']=='editpic'}<em class="bg_0"></em>{/if}<a href="home.php?mod=spacecp&ac=album&op=editpic&albumid=$albumid"{if $_GET['op']!='editpic'} class="f_c"{/if}>{lang edit_pic}</a></li>
		</ul>
	</div>
<!--{/if}-->
<!--{if $_GET['op'] == 'edit'}-->
...
<!--{elseif $_GET['op'] == 'editpic'}-->
	<!--{if $list}-->
		<form method="post" autocomplete="off" id="theform" name="theform" action="home.php?mod=spacecp&ac=album&op=editpic&albumid=$albumid">
			...
			<div class="comiis_showleftbox comiis_act_glfoot comiis_input_style bg_f b_t">
				<div class="comiis_styli cl" style="padding-bottom:5px">                    
					<button type="submit" name="editpicsubmit" value="true" onclick="this.form.action+='&subop=delete';return ischeck('theform', 'ids')" class="comiis_sendbtn bg_del f_f y"><i class="comiis_font z" style="margin-right:3px">&#xe647;</i>{lang delete}</button>
					<button type="submit" name="editpicsubmit" value="true" class="comiis_sendbtn bg_0 f_f y" onclick="this.form.action+='&subop=update';" style="margin-right:10px;">{lang update_explain}</button>	
					<input type="checkbox" name="chkall" id="chkall" />
					<label for="chkall" onclick="checkAll(this.form, 'ids')"><i class="comiis_font"></i>{lang select_all}</label>				
				</div>				
				<div class="comiis_editpic_foot comiis_flex">					
					<!--{if $albumlist}-->
					...
					<div class="comiis_newalbumid flex bg_e b_ok">
