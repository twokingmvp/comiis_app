	...
	<!--{if in_array($filter, array('privatepm','announcepm'))}-->
		...
		<div class="comiis_topnv bg_f b_b">
			<ul class="comiis_flex">
				<li class="flex{if $actives[privatepm]} f_0{/if}">{if $actives[privatepm]}<em class="bg_0"></em>{/if}<a href="home.php?mod=space&do=pm&filter=privatepm"{if !$actives[privatepm]} class="f_c"{/if} comiis_ajax=".comiis_pms_box" comiis_t_ajax=".comiis_topnv">{lang private_pm}</a></li>
				<li class="flex{if $actives[announcepm]} f_0{/if}">{if $actives[announcepm]}<em class="bg_0"></em>{/if}<a href="home.php?mod=space&do=pm&filter=announcepm"{if !$actives[announcepm]} class="f_c"{/if} comiis_ajax=".comiis_pms_box" comiis_t_ajax=".comiis_topnv">{lang announce_pm}</a></li>
			</ul>
		</div>
		<!--{if $comiis_app_switch['comiis_subnv_top'] != 1}--></div></div><!--{/if}-->
	<!--{/if}-->
	<div class="comiis_pms_box">
	<!--{if $filter == 'privatepm'}-->
		<div class="comiis_pmlist mt10 bg_f b_t cl">
			...
			<!--{if $value[pmtype] != 2}-->
			<li>
				<a open_href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_deluid[]=$value[touid]&uid=$value[touid]&handlekey=pmdeletehk_{$plid}&formhash={FORMHASH}&deletesubmit=1" id="a_pmdeleteall_{$value[touid]}" title="{lang delete_privatepm_user}" href="javascript:popup.open('{$comiis_lang['tip388']}', 'confirm', 'javascript:comiis_delpm(\'#a_pmdeleteall_{$value[touid]}\');');" class="kmdel b_ok bg_e f_c"><i class="comiis_font">&#xe67f;</i></a>
	...
	<!--{elseif $filter == 'announcepm'}-->
		<!--{if $count || $grouppms}-->
			<!--{if $grouppms}-->
				...
				<li>
         <a href="javascript:popup.open('{$comiis_lang['tip388']}', 'confirm', 'javascript:comiis_delpm(\'#a_pmdeletesys_{$grouppm['id']}\');');" open_href="home.php?mod=spacecp&ac=pm&op=delete&deletesubmit=1&deletepm_gpmid[]=$grouppm['id']&formhash={FORMHASH}" class="kmdel b_ok bg_e f_c" id="a_pmdeletesys_{$grouppm['id']}"><i class="comiis_font">&#xe67f;</i></a>
...
<!--{if $_GET['viewall'] == '1'}-->
		...
		<!--{else}-->
            <div class="comiis_msg_alldel f_f cl"><span class="bg_del"><a open_href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_deluid[]=$touid&uid=$touid&handlekey=pmdeletehk_{$plid}&formhash={FORMHASH}&deletesubmit=1" id="a_pmdelete_$plid" title="{lang delete_privatepm_user}" href="javascript:popup.open('{$comiis_lang['tip388']}', 'confirm', 'javascript:comiis_delpm(\'#a_pmdelete_$plid\');');"><i class="comiis_font">&#xe67f</i> {lang pm_delete_all}</a></span></div>            
			<!--{loop $result $key $value}-->
				<!--{if $value[msgfromid] != $_G['uid']}-->
				<div class="comiis_friend_msg comiis_msgdel cl">
                    <div class="kmdel f_f">
                        <a open_href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_pmid[]=$value[pmid]&touid=$touid&deletesubmit=1&loc=1&formhash={FORMHASH}" id="a_pmdelete_$value[pmid]" title="{lang delete}" href="javascript:popup.open('{$comiis_lang['tip387']}', 'confirm', 'javascript:comiis_delpm(\'#a_pmdelete_$value[pmid]\');');comiis_closepm();">{lang delete}</a>
                        <!--{if $value['authorid'] && $value['authorid'] != $_G['uid'] && $_G['setting']['pmreportuser']}-->
                            <a href="home.php?mod=spacecp&ac=pm&op=pm_report&pmid=$value[pmid]&handlekey=pmreporthk_{$value[pmid]}" id="a_pmreport_$value[pmid]" title="{lang pmreport}" class="dialog" onclick="comiis_closepm();">{lang pmreport}</a>
                        <!--{/if}-->
                    </div> 
				...
				<!--{else}-->
				<div class="comiis_self_msg comiis_msgdel cl">
                    <div class="kmdel f_f">
                        <a open_href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_pmid[]=$value[pmid]&touid=$touid&deletesubmit=1&loc=1&formhash={FORMHASH}" id="a_pmdelete_$value[pmid]" title="{lang delete}" href="javascript:popup.open('{$comiis_lang['tip387']}', 'confirm', 'javascript:comiis_delpm(\'#a_pmdelete_$value[pmid]\');');comiis_closepm();">{lang delete}</a>
                    </div>
		...
		<!--{/if}-->
<script>
function comiis_closepm() {
	$('.comiis_msgdel').removeClass('comiis_openmsgtip');
	comiis_menu_hide();
	return false;
}
$(document).on('click', '.comiis_msgdel', function() {
	var obj = $(this);
	Comiis_MENU_on = 1
	popup.close();
	obj.addClass('comiis_openmsgtip');
	$('#comiis_menu_bg').off().on('click', function() {
		comiis_closepm();
	}).css({
		'display':'block',
		'width':'100%',
